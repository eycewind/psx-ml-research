...
def _prepare(paths,manifest):
    ...
    return out
def run(config_path:Path,repo:Path,allow_final_holdout=False,verification=None):
    started=time.perf_counter(); cfg=load_config(config_path); paths,c8,input_provenance=validate_inputs(repo,cfg,allow_final_holdout); rows=_prepare(paths,c8); reference_manifest_path=repo/"artifacts/c9/reference_rank_manifest.json"; reference_provenance=json.loads(reference_manifest_path.read_text()) if reference_manifest_path.exists() else None
...
def _reports(root,m,selection,policies,agreement,turnover,baselines):
    ...
    ranks=m["ranking_summaries"]
    (root/"C9_SELECTION_REPORT.md").write_text("# C9 Candidate Selection Report\n\nPredictive candidate outcomes only; no portfolio accounting or profitability claim. Candidate outcomes align to the accepted forward market-relative target window.\n\n## Predefined policies\n\n"+table(policies,cols)+"\n\n## Required rank references\n\n"+table(ranks,["horizon","feature_variant","model_name","date_count","precision_at_5","precision_at_10","precision_at_20","ndcg_at_5","ndcg_at_10","ndcg_at_20","top_decile_capture","bottom_decile_rejection","mrr_actual_top_decile"])+"\n\nThe full 1,218-row threshold, schedule, liquidity and sector grid is retained in `artifacts/c9/selection_metrics.parquet`.\n")
    (root/"C9_ROBUSTNESS_REPORT.md").write_text("# C9 Robustness Report\n\nAll folds, thresholds, schedules, liquidity screens and sector caps retain explicit counts. C8 regime labels are preserved as accepted quantile labels.\n\n## Frozen robustness gates\n\n"+json.dumps(m["robustness_gates"],indent=2,sort_keys=True)+"\n\n## Nearby and structural comparisons\n\n"+table(m["robustness_comparators"],["comparison","mean_outcome","positive_all_folds","positive_ci"])+"\n\n## Concentration leave-outs\n\n"+table(m["concentration_robustness"],["dimension","group_count","base_row_mean","top_5_positive_contribution_share","top_10_positive_contribution_share","top_20_positive_contribution_share","leave_top_1_out_mean","leave_top_3_out_mean","leave_top_5_out_mean","leave_top_10_out_mean"])+"\n\n## Bottom-tail avoidance\n\n"+table(m["bottom_avoidance"],["excluded_fraction","excluded_rows","bottom_mean_outcome","remaining_universe_mean_outcome"])+"\n")
    (root/"C9_TURNOVER_REPORT.md").write_text("# C9 Turnover and Persistence Report\n\nTurnover is diagnostic; costs belong to C10.\n\n## Rank persistence\n\n"+table(m["persistence"],["lag_sessions","rank_autocorrelation","top_decile_persistence","bottom_decile_persistence","selected_falling_below_median","unselected_entering_top_tail"])+"\n\n## Policy turnover sample\n\n"+table(turnover[:100],["policy_id","trade_date","candidate_count","retained","entries","exits","jaccard","gross_candidate_turnover"])+"\n")
    (root/"C9_MODEL_AGREEMENT_REPORT.md").write_text("# C9 Model Agreement Report\n\n## Aggregate\n\n```json\n"+json.dumps(m["model_agreement_summary"],indent=2,sort_keys=True)+"\n```\n\n## Date sample\n\n"+table(agreement[:100],["trade_date","fold_id","rank_correlation","top_5_overlap","top_10_overlap","top_20_overlap","top_decile_overlap","bottom_decile_overlap"])+"\n")
    ...
    (root/"C9_POLICY_DECISION.md").write_text(f"# C9 Policy Decision\n\nStatus: **{m['decision_status']}**\n\nPrimary: `{m['selected_primary_policy']}`\n\nConservative: `{m['selected_conservative_policy']}`\n\n## Acceptance gates\n\n```json\n{json.dumps(m['robustness_gates'],indent=2,sort_keys=True)}\n```\n\nP1 and P2 are frozen candidate-selection rules for a later fee-aware C10 evaluation. This is not authorization to infer implementable profitability. P3 remains a diagnostic and is not selected. The 2026 holdout remains locked.\n")
    p1=next(r for r in policies if r["policy_id"]=="P1_broad_canonical"); p2=next(r for r in policies if r["policy_id"]=="P2_conservative_consensus"); random=next(r for r in baselines if r["baseline"]=="random_same_count")
    (root/"C9_DELIVERY.md").write_text(f"# C9 Delivery Report\n\nStatus: **{m['decision_status']}**\n\nGeneration commit: `{m['code']['commit']}`; dirty: **{m['code']['dirty']}**; holdout accessed: **false**.\n\n## Decision\n\nFreeze `P1_broad_canonical` as the primary C10 candidate and `P2_conservative_consensus` as the conservative alternative. P1 has mean date-level market-relative outcome `{p1['mean_outcome']:.8g}`, spread versus unselected `{p1['mean_spread']:.8g}`, positive-date fraction `{p1['positive_date_fraction']:.4f}`, and a 95% outcome interval `[{p1['outcome_ci']['lower_95']:.8g}, {p1['outcome_ci']['upper_95']:.8g}]`. P2 has mean outcome `{p2['mean_outcome']:.8g}` and interval `[{p2['outcome_ci']['lower_95']:.8g}, {p2['outcome_ci']['upper_95']:.8g}]`. Both are positive in all three folds.\n\nP1 exceeds the deterministic same-count random mean `{random['mean_outcome']:.8g}` with empirical p-value `{random['empirical_p_value']:.6g}`. Nearby 5% and 20% thresholds, L1/L2 liquidity screens, S0/S2 sector constraints, XGBoost, and the non-overlapping schedule retain positive fold and bootstrap gates. Date, symbol, and sector leave-outs remain positive. Mean LightGBM/XGBoost rank correlation is `{m['model_agreement_summary']['rank_correlation']:.6g}`.\n\nC9 evaluates predictive candidate-selection robustness only. It does not include fees, spreads, slippage, portfolio accounting, P&L, Sharpe, drawdown, live signals or a profitability claim. The final 2026 holdout remains untouched.\n")
def main():
    p=argparse.ArgumentParser(); p.add_argument("--config",type=Path,required=True); p.add_argument("--repo",type=Path,default=Path.cwd()); p.add_argument("--allow-final-holdout",action="store_true"); p.add_argument("--cpu-result"); p.add_argument("--gpu-result"); a=p.parse_args(); verification={"cpu_suite":a.cpu_result,"gpu_suite":a.gpu_result} if a.cpu_result and a.gpu_result else None; m=run(a.config,a.repo,a.allow_final_holdout,verification); print(f"C9 {m['decision_status']}: {m['counts']['selection_metric_rows']} policy comparisons; holdout=false")
