...
def run(config_path:Path,repo:Path,allow_final_holdout=False):
    ...
    manifest={"manifest_version":1,"generated_at_utc":datetime.now(timezone.utc).isoformat(),"code":code,"holdout_accessed":False,"canonical_universe":CANONICAL,"sector_provenance":"2026-08-01 security-master historical backcast","benchmark_definitions":{"market":"same-date eligible ordinary-equity leave-one-out median","sector":"same-date same-sector eligible ordinary-equity leave-one-out median","sector_minimum_policy":raw["target"]["sector_minimum_policy"],"minimum_sector_peers":raw["target"]["minimum_sector_peers"]},"target_definitions":list(derived),"feature_definitions":{"market":market_names,"sector":sector_names,"relative":relative_names,"ret_10obs":"compound current ret_5obs with the same symbol's five-observation-lag ret_5obs","rolling_window":60,"minimum_rolling_observations":30},"counts":counts,"sector_exclusion_summary":audit_summary,"inputs":{k:{"path":str(v),"sha256":sha256_file(v)} for k,v in paths.items()},"outputs":{"relative_targets":{"path":str(out),"file_sha256":sha256_file(out),"logical_sha256":logical_hash(table)},"sector_audit":{"path":str(audit_path),"file_sha256":sha256_file(audit_path),"logical_sha256":logical_hash(audit_table)},"sector_coverage":{"path":str(coverage_path),"file_sha256":sha256_file(coverage_path),"logical_sha256":logical_hash(coverage_table)},"feature_missingness":{"path":str(missing_path),"file_sha256":sha256_file(missing_path),"logical_sha256":logical_hash(missing_table)},**feature_outputs}}
