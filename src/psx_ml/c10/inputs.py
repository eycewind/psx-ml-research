...
def load_c9_selections(path: Path = C9_SELECTIONS_PATH) -> pd.DataFrame:
    frame = normalize_dates(pd.read_parquet(path))

    required = {
        "policy_id", "trade_date", "symbol", "fold_id", "horizon",
        "target_family", "feature_variant", "model_name", "prediction",
        "sector", "selection_date", "selection_tail",
    }
    missing = sorted(required - set(frame.columns))
    if missing:
        raise ValueError(f"C9 selections missing required columns: {missing}")
    ...
    available_policies = set(frame["policy_id"].dropna().unique())
    missing_policies = sorted(valid_policies - available_policies)
    if missing_policies:
        raise ValueError(
            f"Required frozen C9 policies are missing: {missing_policies}"
        )

    frame = frame.loc[frame["policy_id"].isin(valid_policies)].copy()
