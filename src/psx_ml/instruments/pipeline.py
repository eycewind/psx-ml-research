...
def run_foundation(repo: Path, instruments_path=Path("config/instruments.yaml"), universe_path=Path("config/universe_c6.yaml")):
    ...
    robust_report += ["","Stratified fold, year, liquidity-bucket, stale-bucket, and instrument-family rows are preserved in `c6_robust_metrics.parquet`. Canonical untrimmed metrics remain visible beside robust diagnostics.","","## C7 universe decision","","The decision is generated after inspecting the live stability and ranking diagnostics; lower RMSE alone is not sufficient.",""]
