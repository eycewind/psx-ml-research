...
def _aggregate(metrics):
    ...
    for (target,model),rows in sorted(grouped.items()):
        names=sorted(set.intersection(*(set(k for k,v in r.items() if isinstance(v,(int,float)) and not isinstance(v,bool) and k!="n") for r in rows)))
        mean={}; std={}; finite_counts={}
        for k in names:
            values=np.asarray([r[k] for r in rows],float); finite=values[np.isfinite(values)]
            mean[k]=float(np.mean(finite)) if finite.size else None; std[k]=float(np.std(finite)) if finite.size else None; finite_counts[k]=int(finite.size)
        out[f"{target}:{model}"]={"folds":len(rows),"mean":mean,"std":std,"finite_fold_counts":finite_counts,"total_n":sum(r["n"] for r in rows)}
    return out
...
def _report(manifest,path):
    ...
    lines += ["","## Finite daily IC coverage","","Undefined same-date IC is excluded, never coerced to zero.","","| Target/fold/model | Validation dates | Population eligible | Finite IC | Constant prediction | Constant target | Non-finite IC |","|---|---:|---:|---:|---:|---:|---:|"]
    for k,v in sorted(manifest["per_fold_metrics"].items()):
        if "finite_ic_date_count" in v: lines.append(f"| `{k}` | {v['validation_date_count']} | {v['population_eligible_date_count']} | {v['finite_ic_date_count']} | {v['constant_prediction_date_count']} | {v['constant_target_date_count']} | {v['nonfinite_ic_date_count']} |")
...
def run_pipeline(config_path,repo,allow_final_holdout=False):
    ...
    runtime=["# C7 Runtime Report","",f"Canonical CPU mode uses one thread; XGBoost verification uses CUDA on `{manifest['device']['gpu']}`.","","## Fit and prediction runtime","","| Task | Fold | Model | Device | Fit s | Predict s | Rounds | Train rows | Validation rows |","|---|---|---|---|---:|---:|---:|---:|---:|"]+[f"| `{r['target']}` | `{r['fold']}` | `{r['model']}` | `{r['device']}` | {r['fit_seconds']:.4f} | {r['predict_seconds']:.4f} | {r['rounds']} | {r['train_rows']} | {r['validation_rows']} |" for r in evidence["runtimes"]]
    runtime += ["","## Early stopping and prediction diagnostics","","| Task/fold/model | Best iter | Metric | Best inner | First | Last evaluated | Mean | Std | Min | Max | Unique | Near constant |","|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---|"]
    for r in evidence["runtimes"]: runtime.append(f"| `{r['target']}:{r['fold']}:{r['model']}` | {r['best_iteration']} | `{r['early_stopping_reference_metric']}` | {r['best_inner_validation_score']} | {r['first_iteration_score']} | {r['last_evaluated_iteration']} | {r['prediction_mean']:.8g} | {r['prediction_std']:.8g} | {r['prediction_min']:.8g} | {r['prediction_max']:.8g} | {r['unique_prediction_count']} | {r['constant_or_near_constant_prediction']} |")
    cfg.path("output","runtime_report_path").write_text("\n".join(runtime)+"\n")
