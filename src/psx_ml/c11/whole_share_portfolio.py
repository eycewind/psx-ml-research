...
def build_whole_share_portfolio(
    *,
    policy_id: str,
    mapped_selections: pd.DataFrame,
    prices: pd.DataFrame,
    config: WholeShareConfig,
) -> PortfolioResult:
    ...
    for valuation_date_value in valuation_dates:
        ...
        if not holdings:
            # Keep a NAV row if a rebalance occurred but every selected share was unaffordable.
            if rebalance_event is not None:
                nav_close = cash
                ...
                nav_rows.append({
                    ...
                    "purchasable_target_count": purchasable_target_count,
