...
def _model_report(m,path):
    lines=["# C5 Linear Baseline Model Report","",f"Model set `{m['model_set']['name']}` v{m['model_set']['version']}; final holdout accessed: **{m['holdout_accessed']}**.","",
...
def run_pipeline(config_path,repo,allow_final_holdout=False):
    ...
    agg=_aggregate(evidence["metrics"]); cs=_coef_summary(coefficients)
    manifest={"manifest_version":1,"generated_at_utc":datetime.now(timezone.utc).isoformat(),"code":git_state(repo),"model_set":{"name":c.name,"version":c.version},
      ...
      "date_block_intervals":evidence["date_block_intervals"],"coefficient_summary":cs,"configuration":c.canonical(),"configuration_sha256":c.sha256(),
      ...
      "outputs":{"predictions":{"path":str(c.predictions_path),"file_sha256":sha256_file(c.predictions_path),"logical_sha256":logical_hash(predictions)},
                 "coefficients":{"path":str(c.coefficients_path),"file_sha256":sha256_file(c.coefficients_path),"logical_sha256":logical_hash(coefficients)}},"packages":runtime_versions()}
